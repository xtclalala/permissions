...
// Register 注册用户
func (s *M2MUserService) Register(dto system.SysUser) (err error) {
...
// Search 搜索用户
func (s *M2MUserService) Search(dto SearchUser) (err error, list []system.SysUser, total int64) {
	limit := dto.PageSize
...
func (s *M2MUserService) CheckRepeat(loginName string) (err error) {
...
// GetAll 查所有用户
func (s *M2MUserService) GetAll() (err error, dos []system.SysUser) {
	err = global.Db.Find(&dos).Error
	return
}

// GetById 根据 id 查用户
func (s *M2MUserService) GetById(id uint) (err error, do system.SysUser) {
